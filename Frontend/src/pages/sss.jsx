...
const StudentCameraComponent = React.memo(({ 
  localStream, 
  cameraActive,
  isSharingCamera,
  cameraRequested
  ...
  return (
    <div className="student-camera-container">
      <div className="camera-header">
        <h4>📹 Exam Camera</h4>
        <span className={`camera-status ${cameraActive ? 'active' : 'inactive'}`}>
      ...
      <div className="camera-preview">
        {localStream ? (
          ...
          <div className="camera-placeholder">
            ...
            <p>{cameraRequested ? 'Camera access requested...' : 'Camera not active'}</p>
            {cameraRequested && !localStream && (
              <small>Teacher may request camera access</small>
            )}
          </div>
        )}
      </div>
      ...
      <div className="camera-info">
        {isSharingCamera && (
          <div className="sharing-status">
            ...
            <small>Streaming to teacher</small>
          </div>
        )}
        <small>This camera is for proctoring purposes only.</small>
      </div>
    </div>
  );
});
...
// ==================== UPDATED: INITIAL EXAM ENTRY CHECK ====================
// Line ~1150: Palitan ang useEffect na ito
useEffect(() => {
  const checkIfCanEnter = async () => {
    try {
      ...
      if (statusResponse.success) {
        const { canEnter, isActive, isStarted, examType } = statusResponse.data;
        
        console.log('Exam status:', { canEnter, isActive, isStarted, examType });
        
        if (canEnter && isActive && isStarted) {
          // LIVE CLASS: Auto-start camera immediately
          if (examType === 'live-class' && requiresCamera) {
            console.log('🚀 LIVE CLASS: Auto-starting camera...');
            try {
              const stream = await navigator.mediaDevices.getUserMedia({ 
                video: { 
                  width: { ideal: 640 },
              ...
              console.log('✅ Camera auto-started for live class');
              setLocalStream(stream);
              setCameraActive(true);
              setCameraRequested(true);
              
              // Store stream for immediate sharing when teacher requests
              window.studentCameraStream = stream;
              
            } catch (error) {
              console.warn('⚠️ Camera auto-start failed:', error);
            }
          }
          ...
          if (requiresMicrophone) {
            try {
              ...
              alert('🎤 Microphone access is REQUIRED for live class.');
              return;
            }
          }
...
}, [examId, requiresMicrophone, requiresCamera, loadQuiz]);
 ...
 const handleExamStart = useCallback(async () => {
  try {
    ...
    // Auto-start camera kung required
    if (requiresCamera) {
      ...
      try {
        ...
        // Auto-share with teacher kung may socket connection
        ...
        // Ipaalam sa user pero hayaan pa ring mag-proceed
        alert('⚠️ Camera could not start automatically. You can try manually later.');
      }
    }
...
}, [requiresMicrophone, requiresCamera, loadQuiz, teacherSocketId]);

  // ==================== SOCKET.IO SETUP ====================
useEffect(() => {
  const token = localStorage.getItem('token');
  
  // ✅ KUHAIN ANG USER ID MULA SA TOKEN O LOCAL STORAGE
  let userId = 'student-user'; // Default
  
  try {
    // Subukan i-decode ang JWT token para makuha ang user ID
    if (token) {
      const payload = JSON.parse(atob(token.split('.')[1]));
      userId = payload.id || 'student-user';
    }
  } catch (error) {
    console.warn('Could not decode token, using default userId');
  }
  
  // ✅ PAGKATAPOS, KUNIN MULA SA LOCAL STORAGE KUNG MAY NAKASTORE
  ...
  console.log('👤 User ID for socket:', userId); // ✅ DEBUG

  const newSocket = io('http://localhost:3000', {
    auth: { 
      token: token,
      userId: userId  // ✅ GAMITIN ANG NATUKOY NA USER ID
    },
    query: { 
      examId: examId,
      userRole: 'student',
      userId: userId  // ✅ ISAMA SA QUERY PARAMETERS
    },
...
const handleProctoringAlert = useCallback((alertData) => {
  ...
  // ✅ LIST OF ALL PYTHON DETECTIONS THAT SHOULD DEDUCT ATTEMPTS
  const pythonDetectionTypes = [
    'no_face_detected',
    'multiple_people',
    'gaze_deviation',
    'head_pose_deviation',
    ...
    'pointing_gesture',
    'screenshot_detected',
    'screenshot_tool_detected',
    'screen_capture_artifacts',
    'multiple_screen_indicators',
    'mouse_usage'
  ];
  
  const pythonDetectionMessages = [
    'NO FACE',
    'MULTIPLE PEOPLE',
    'GAZE',
    'HEAD POSE',
  ...
  // ✅ Check if this is from Python backend
  const isFromPythonBackend = alertData.source === 'python_backend' || 
                             alertData.detectionSource === 'python' ||
                             (alertData.detectionType && pythonDetectionTypes.includes(alertData.detectionType)) ||
                             (alertData.message && pythonDetectionMessages.some(msg => alertData.message.includes(msg))) ||
                             alertData.detectionType?.includes('detected') ||
                             alertData.detectionType?.includes('detection');
  
  console.log('🐍 Python detection check:', {
    message: alertData.message,
    detectionType: alertData.detectionType,
    isFromPythonBackend: isFromPythonBackend,
    source: alertData.source,
    detectionSource: alertData.detectionSource
  });
  
  // ✅ DEDUCT ATTEMPT FOR ALL PYTHON DETECTIONS
  if (isFromPythonBackend) {
    console.log('🐍 Python backend detection - DEDUCTING ATTEMPT');
    
    // Check if teacher has disabled this specific detection
    const isAlertTypeEnabled = (detectionType) => {
      if (!detectionType) return true;
      
      const settingMap = {
        'gaze_deviation': teacherDetectionSettings.gazeDetection,
        'head_pose_deviation': teacherDetectionSettings.gazeDetection,
        'no_face_detected': teacherDetectionSettings.faceDetection,
        ...
        'speaking_detected': teacherDetectionSettings.audioDetection,
        'suspicious_gesture': teacherDetectionSettings.handGestureDetection,
        'hand_covering_face': teacherDetectionSettings.handGestureDetection,
        'pointing_gesture': teacherDetectionSettings.handGestureDetection,
        'screenshot_detected': teacherDetectionSettings.screenshotDetection,
        'mouse_usage': teacherDetectionSettings.phoneDetection
      };
    ...
    if (detectionType && !isAlertTypeEnabled(detectionType)) {
      console.log(`🛑 Alert type ${detectionType} disabled by teacher - NOT deducting attempt`);
      return;
    }
    ...
    setStudentAttempts(prev => {
      ...
      console.log(`🐍 Python Attempts Update: ${prev.currentAttempts} → ${newAttempts} (${alertData.detectionType || alertData.message})`);
      ...
      const updated = {
        ...
        history: [
          ...
          {
            timestamp: new Date().toISOString(),
            violation: detectionType || alertData.message,
            message: alertData.message,
            attemptsLeft: attemptsLeft,
      ...
      // Alert kung mababa na ang attempts
      if (attemptsLeft <= 3 && attemptsLeft > 0) {
        alert(`⚠️ Warning: Only ${attemptsLeft} attempt(s) left!`);
      }
  ...
  // Store alert regardless of attempts deduction
  const newAlert = {
    id: Date.now(),
    message: alertData.message || 'Suspicious activity detected',
    timestamp: new Date().toLocaleTimeString(),
...
// ==================== DETECTION LOGIC - ADD THIS ====================
useEffect(() => {
  if (!examStarted || !permissionsGranted || !requiresCamera || !localStream) return;
  
  console.log('🎯 Starting detection loop with settings:', teacherDetectionSettings);
  ...
  const detectionInterval = setInterval(async () => {
    try {
      ...
      const detectionSettings = {
        faceDetection: teacherDetectionSettings.faceDetection,
        gazeDetection: teacherDetectionSettings.gazeDetection,
        phoneDetection: teacherDetectionSettings.phoneDetection,
        mouthDetection: teacherDetectionSettings.mouthDetection,
        multiplePeopleDetection: teacherDetectionSettings.multiplePeopleDetection,
        audioDetection: teacherDetectionSettings.audioDetection,
        handGestureDetection: teacherDetectionSettings.handGestureDetection,
        tabSwitchDetection: teacherDetectionSettings.tabSwitchDetection,
        screenshotDetection: teacherDetectionSettings.screenshotDetection,
        attentionDetection: teacherDetectionSettings.attentionDetection
      };
      ...
      const response = await fetch('http://localhost:5000/detect', {
        ...
        body: JSON.stringify({
          image: imageBase64,
          exam_id: examId,
          student_socket_id: socketRef.current?.id,
          student_id: 'student-user',
      ...
      if (response.ok) {
        ...
        // Sa detection interval useEffect (~line 1560-1650)
// Sa detection interval useEffect
if (result.suspiciousActivities && result.suspiciousActivities.length > 0) {
  console.log('🚨 Detected suspicious activities:', result.suspiciousActivities);
  
  // Handle detections
  result.suspiciousActivities.forEach(activity => {
    // Determine detection type based on activity message
    let detectionType = 'unknown';
    let severity = 'medium';
    let type = 'warning';
    
    if (activity.includes('NO FACE')) {
      detectionType = 'no_face_detected';
      severity = 'high';
      type = 'danger';
    } else if (activity.includes('MULTIPLE PEOPLE')) {
      detectionType = 'multiple_people';
      severity = 'high';
      type = 'danger';
    } else if (activity.includes('GAZE')) {
      detectionType = 'gaze_deviation';
      severity = 'medium';
      type = 'warning';
    } else if (activity.includes('HEAD POSE')) {
      detectionType = 'head_pose_deviation';
      severity = 'medium';
      type = 'warning';
    } else if (activity.includes('MOUTH')) {
      detectionType = 'mouth_movement';
      severity = 'medium';
      type = 'warning';
    } else if (activity.includes('PHONE')) {
      detectionType = 'phone_usage';
      severity = 'high';
      type = 'danger';
    } else if (activity.includes('SCREENSHOT')) {
      detectionType = 'screenshot_detected';
      severity = 'high';
      type = 'danger';
    } else if (activity.includes('MOUSE')) {
    ...
    const newAlert = {
      ...
      source: 'python_backend' // ✅ CRITICAL: Mark as from Python
    };
    
    // ✅ CRITICAL: Call handleProctoringAlert for ALL detections
    handleProctoringAlert(newAlert);
    
    // Also send to teacher via socket
    if (socketRef.current && result.faceCount > 0) {
      socketRef.current.emit('proctoring-alert', {
        ...
        severity: severity,
        detectionType: detectionType,
        confidence: result.detectionConfidence || 0.7,
        timestamp: new Date().toISOString(),
        source: 'python_backend' // ✅ CRITICAL: Include source
      });
    }
  });
}
...
}, [examStarted, permissionsGranted, requiresCamera, localStream, examId, teacherDetectionSettings, socketRef]);
...
const testCamera = async () => {
  ...
  return (
    ...
    <span className="attempts-used">
      Used: {studentAttempts.currentAttempts}
    </span>
...
{/* Camera Component */}
{/* Sa main render function, palitan ang camera component rendering */}
